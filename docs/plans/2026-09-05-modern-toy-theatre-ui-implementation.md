...
| Nodes and path | Tactile ivory rim/soft depth, teal locator/check, muted lock and ivory path retained. Desktop 220/244px and mobile 140/152px; real three-lesson chapters replace the concept's arbitrary extra numbered discs. | Sign off; scanner 0/30 uses honest empty numbered nodes instead of invented child images. |
| Topbar density | Floating brand/menu, real progress and account are separate ivory groups. Current action stays upper-right on desktop and bottom on mobile. | Intentional lighter flat controls and real app name; no thick embossed frame, reserved Web window controls or fictional avatar. |
...
| Colors, shadows and frames | Actual shared tokens are #fffaf2, #402b20, #267f7b, #ffddb0 and #ded2c3; shadows are restrained and reading body has no enclosing outer card. | Sign off against the written token system and explicit instruction to avoid heavy frames. |
