...
def _continuous_lesson(move_id):
    lesson = valid_lesson(move_id, v2=True)
    partner = lesson["roles"][1]
    lesson["dialogue_contract"] = "continuous-dialogue-v1"
    lesson["dialogue"] = {
        "turns": [
            {"speaker": partner, "line": "Snack time. Two choices are ready."},
            {"speaker": "child", "line": "Can I have the apple, please?"},
            {"speaker": partner, "line": "Sure. One second."},
            {"speaker": "child", "line": "Okay, I'll wait."},
            {"speaker": partner, "line": "Here you go."},
            {"speaker": "child", "line": "Thank you. I'll eat at the table."},
        ]
    }
    lesson.pop("parts")
    return lesson
...
def test_complete_stage_requires_every_lesson_to_migrate_but_incremental_accepts_v1():
    ...
    # Completing Stage 04 now requires the continuous contract, so both the
    # legacy lesson and the three-by-ten lesson report the migration gap.
    assert [issue.code for issue in issues] == ["dialogue-contract", "dialogue-contract"]

    stage["chapters"][0]["lessons"][0] = _continuous_lesson("request-item")
    stage["chapters"][0]["lessons"][1] = _continuous_lesson("second-move")
...
def test_stage_04_declares_the_completed_continuous_dialogue_contract():
    ...
    assert stage["dialogue_contract"] == "continuous-dialogue-v1"
    assert len(lessons) == 30
    assert all(lesson["dialogue_contract"] == "continuous-dialogue-v1" for lesson in lessons)
    assert all("parts" not in lesson for lesson in lessons)
...
def test_chapter_1_builds_requests_through_recycled_moves():
    ...
    assert all(lesson["dialogue_contract"] == "continuous-dialogue-v1" for lesson in lessons)
    assert all("parts" not in lesson for lesson in lessons)
    assert all(len(lesson["dialogue"]["turns"]) >= 5 for lesson in lessons)
    assert validate_stage(load_stage(CURRICULUM_ROOT, "04")) == []


CONTINUOUS_KEY_PHRASES = {
    ("01-choosing-requests", "01-request-an-item"): "Can I have the apple",
    ("01-choosing-requests", "02-specify-a-choice"): "I want the dinosaur one",
    ("01-choosing-requests", "03-change-a-choice"): "Actually, can I have water instead",
    ("02-refusal-negotiation", "01-not-ready-yet"): "I don't want to stop yet",
    ("02-refusal-negotiation", "02-ask-for-time"): "Can I have two more minutes",
    ("02-refusal-negotiation", "03-propose-an-order"): "What if we do blocks first",
    ("03-help-clarification", "01-ask-for-help"): "Can you help me",
    ("03-help-clarification", "02-say-i-dont-understand"): "I don't understand",
    ("03-help-clarification", "03-correct-a-misunderstanding"): "No, I mean the blue cup",
    ("04-body-needs", "01-hungry-or-thirsty"): "I'm thirsty",
    ("04-body-needs", "02-request-a-pause"): "I need to use the bathroom",
    ("04-body-needs", "03-say-what-hurts"): "My knee hurts",
    ("05-routines-transitions", "01-first-then"): "First my shoes, then my jacket",
    ("05-routines-transitions", "02-before-bed"): "Can we read the bear book before bed",
    ("05-routines-transitions", "03-check-readiness"): "I'm ready, but I still need my water bottle",
    ("06-finding-belonging", "01-ask-where"): "Where's my red car",
    ("06-finding-belonging", "02-check-a-place"): "Is it under the bed",
    ("06-finding-belonging", "03-say-whose"): "Mine has a green dinosaur sticker",
    ("07-joining-cooperation", "01-join-play"): "Can I play with you",
    ("07-joining-cooperation", "02-ask-for-a-turn"): "Can I have a turn after you",
    ("07-joining-cooperation", "03-plan-the-game"): "How about we play space pirates",
    ("08-feelings-repair", "01-feeling-and-reason"): "I'm frustrated because",
    ("08-feelings-repair", "02-ask-to-stop"): "Please stop. I don't like that",
    ("08-feelings-repair", "03-apologize-and-repair"): "I'm sorry",
    ("09-outings-safety", "01-how-much-longer"): "How much longer",
    ("09-outings-safety", "02-find-it-in-a-shop"): "Where can we find the pasta",
    ("09-outings-safety", "03-safe-adult-help"): "I can't find my mom",
    ("10-recounting-planning", "01-one-event-today"): "I built a big tower with Maya",
    ("10-recounting-planning", "02-first-and-then"): "First we painted",
    ("10-recounting-planning", "03-plan-tomorrow"): "Tomorrow we're going to the park",
}
...
@pytest.mark.parametrize(("chapter_id", "lesson_id"), sorted(CONTINUOUS_KEY_PHRASES))
def test_all_lessons_meet_continuous_dialogue_contract(chapter_id, lesson_id):
    stage = load_stage(CURRICULUM_ROOT, "04")
    chapter = next(c for c in stage["chapters"] if c["id"] == chapter_id)
    lesson = next(l for l in chapter["lessons"] if l["id"] == lesson_id)
    production = json.loads(
        (REPO_ROOT / "prompts" / "04" / chapter_id / lesson_id / "production.json").read_text(encoding="utf-8")
    )
    turns = lesson["dialogue"]["turns"]
    child_turns = [t for t in turns if t["speaker"] == "child"]

    assert lesson["dialogue_contract"] == "continuous-dialogue-v1"
    assert "parts" not in lesson
    for field in ("core_response", "stretch_response", "repair_response"):
        assert field not in lesson
    assert len(turns) >= 5
    assert len(child_turns) >= 3
    assert {t["speaker"] for t in turns} == set(lesson["roles"])
    assert all(t["line"].strip() for t in turns)
    assert any(
        CONTINUOUS_KEY_PHRASES[(chapter_id, lesson_id)] in t["line"]
        for t in child_turns
    )
    assert len(lesson["replay_cards"]) == 2
    assert lesson["status"] == "language_reviewed"
    assert all(lesson["reviews"].get(review) is True for review in TEN_REVIEWS)

    clips = production["clips"]
    assert len(clips) == 3
    assert [i for clip in clips for i in clip["turns"]] == list(range(len(turns)))
    assert all(all(clip.get(field) for field in ("start", "action", "end")) for clip in clips)
    assert lesson["content_revision"] == production["content_revision"]
