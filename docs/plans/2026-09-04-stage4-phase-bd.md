...
- [x] Add a failing outline test for exact Chapter, Lesson, Move, and partner order across Chapters 4–10.
- [x] Author Chapter 4 with needs that cause a useful adult response; medical language stays descriptive, not diagnostic.
- [x] Author Chapter 5 with genuine household transitions and clear, finite boundaries.
- [x] Author Chapter 6 so every location question has an information gap and every ownership claim has evidence.
- [x] Author Chapter 7 with a same-age peer; joining and turn-taking allow a real response rather than scripted compliance.
- [x] Author Chapter 8 with valid feelings, immediate stop boundaries, and repair that changes the shared activity.
- [x] Author Chapter 9 with minimal set dressing; the lost-child lesson is explicitly a calm home rehearsal using a named safe adult role.
- [x] Author Chapter 10 with age-appropriate recounting: one event, then two linked events, then one contribution to a family plan.
- [x] Run incremental validator after every Chapter and correct word budgets, recycling order, and partner ratios.
- [x] Create `curriculum/04/FINAL-REVIEW.md` with per-Lesson word counts, move coverage, authenticity exceptions, and release decision.
- [x] Run `python tools/validate_curriculum.py --stage 04 --complete`, full Python tests, and JavaScript tests.
- [x] If all gates pass, set Stage status to `language_reviewed` and record completed phases A–D. Runtime migration remains a separate task.
