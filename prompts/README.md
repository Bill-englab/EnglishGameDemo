...
迁移按课启用 `dialogue_contract: "three-by-ten-v2"`。未迁移的 v1 课暂时保留原导出模板（包括旧节奏提醒），保证既有 90 份导出在逐章迁移期间可检查；新版男孩角色、情绪和十秒约束只应用于 v2。普通课程校验允许旧课，新版 Stage 04 的 `--complete` 在全部 30 课迁移前会有意失败。提示词校验只统计结构化台词，不把场景或情绪说明计入词数。
