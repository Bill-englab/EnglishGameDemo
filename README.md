...
All 30 lessons now use `continuous-dialogue-v1`: each Lesson owns one natural conversation in `dialogue.turns`, while the three ~10-second clips are a production-only split in `production.json`, exported as [90 synchronized a/b/c prompt drafts](prompts/04/README.md) (one file per clip). Speech is exported verbatim from canonical lessons; per-lesson staging preserves physical continuity and bounded emotion. See the [production and maintenance guide](prompts/README.md), the [authoring style guide](docs/specs/2026-09-06-dialogue-authoring-style.md) and the [complete Stage 1 review](curriculum/04/FINAL-REVIEW.md). Prompt availability is not `video_ready` approval or evidence of a finished video.
