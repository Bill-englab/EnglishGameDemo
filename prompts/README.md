...
网页 VideoGen 展示和 Copy 只提供生成内容。文件头里的课程路径、内容版本与 SHA256 属于内部同步校验资料，保留在导出文件中，由接口在返回页面时去掉；不会复制给视频生成工具。直接使用文件时，从 `VIDEO AND SOUND` 开始复制即可。

**[逐课索引 →](04/README.md)**

## 直接使用

1. 在课程详情页点击 **Watch & Learn → VideoGen · Get prompts**，展开 **VideoGen · A/B/C prompts**，分别复制 Part A、B、C。也可直接打开 `04/<章>/<课>/a.txt`、`b.txt`、`c.txt`。
...
**2026-09-07 画风统一：** 恢复最早提示词的 `Pixar-style 3D animated cartoon, soft warm lighting, cozy pastel color palette, cute, wholesome, family-friendly.`，所有角色统一使用柔和圆润的3D卡通外形。孩子和爸爸恢复旧版具体面部特征，妈妈、老师和同伴沿用当前固定物种与服装。大圆眼和胖脸颊是外形设定，不代表惊讶或兴奋；当前逐课 `EMOTIONAL PERFORMANCE`、音量、自然小幅表情与动作约束保持原样，不恢复旧版蹦跳和兴奋摇尾巴。统一画风及外形由 `tools/build_video_prompts.py` 的 `VISUAL_STYLE` / `CAST` 维护，修改后统一导出90份文件。

| 角色 | 固定视觉设定 |
| --- | --- |
| Child | 4 岁小老虎男孩，亮橙色毛发、醒目黑条纹、大而闪亮的圆眼睛、胖圆脸颊、小圆耳朵、黄色 T 恤、矮胖幼儿身形；需要代词时用 he/him |
| Dad | 温棕色狗狗、大垂耳、柔软圆润的口鼻、温柔略带疲惫的眼神、橄榄绿 T 恤 |
